/*
  ...
  - Removed authentication dependencies when WEBSERVER_NO_AUTH defined
  - Removed CORS/ETag/Chunked/SSE/Raw/Middleware when respective flags defined
  - Uri-based on() always compiles, uses SimpleRequestHandler when no REGEX
...
#endif // WEBSERVER_INCLUDE_AUTH

// ===== ROUTE REGISTRATION =====
// Uri-based methods - ALWAYS available
// Uri имеет конструктор от const char*, поэтому on("/path", ...) работает
...
RequestHandler &WebServer::on(const Uri &uri, HTTPMethod method, WebServer::THandlerFunction fn, WebServer::THandlerFunction ufn) {
  ...
  #else
    SimpleRequestHandler *handler = new SimpleRequestHandler(fn, ufn, uri.getUri(), method); 
  #endif
...
#endif // WEBSERVER_INCLUDE_CHUNKED
...
uint32_t WebServer::getLastRequestTime() const {
    return _lastRequestTime;
}
    
// Метод для проверки простоя
uint32_t WebServer::getIdleTime() const {
      return millis() - _lastRequestTime;
}

void WebServer::_updateActivity() { 
